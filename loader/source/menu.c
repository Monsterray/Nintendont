...
typedef struct _MenuCtx
{
	...
	// Settings menu. (see SettingsMenu.c)
 ...
 * Input handling, layout and the option table live in SettingsMenu.c.
 * @param ctx	[in] Menu context.
 * @return True to exit; false to continue.
 */
static bool UpdateSettingsMenu(MenuCtx *ctx)
{
	...
	SettingsMenuInput in;
	in.up      = FPAD_Up_Repeat(ctx);
	in.down    = FPAD_Down_Repeat(ctx);
	...
	if (ctx->redraw)
	{
		SettingsMenu_Draw(&ctx->settings);
		// GRRLIB rendering is done by SelectGame().
	}
	...
	while(1)
	{
		...
		if( FPAD_Cancel(0) )
		{
			...
			if (ctx.menuMode == 1)
			{
				...
				SettingsMenu_Reset(&ctx.settings);
			}
